...
public class IndexBuilder {
    ...
    private void parseData(String fileName) throws IOException {
        try (FileReader dataFile = new FileReader(fileName);
             ...
             FileWriter threeWordPostFile = new FileWriter("./result/ThreeWordPosts");
             ...
             BufferedWriter threeWordPost = new BufferedWriter(threeWordPostFile)) {
            ...
            for (String[] curr : lyrics) {
                ...
                int index = makePost(curr, oneWordPost, twoWordPost, threeWordPost);
                if (index == -1) continue;
    ...
    private int makePost(String[] lyrics, BufferedWriter oneWordPost, BufferedWriter twoWordPost, BufferedWriter threeWordPost) throws IOException{
        String content = parseContent(lyrics[5]);
        String[] words = content.split("\\s");
        List<String> formatedWord = new ArrayList<>(words.length);
        for (String word : words) {
            word = formatWord(word);
            if (word.length() > 0) formatedWord.add(word);
        }
        ...
        String[] threeWords = threeWords(oneWords);
        writePost(oneWords, id, oneWordPost);
        writePost(twoWords, id, twoWordPost);
        writePost(threeWords, id, threeWordPost);
        return ++id;
    }

    private String[] threeWords(String[] words) throws IOException {
        int len = words.length;
        String[] threeWords = new String[len - 2];
        for (int i = 0; i < len - 2; i++) {
            threeWords[i] = words[i] + "-" + words[i + 1] + "-" + words[i + 2];
        }
        return threeWords;
    }
    ...
    private void buildIndex(String dataFile) throws IOException, InterruptedException {
        ...
        process.waitFor();
        process = new ProcessBuilder("/bin/bash", "-c", "sort -t ':' -k 1,1 -k 2n,2 ./result/ThreeWordPosts -o ./result/ThreeWordPosts").start();
        process.waitFor();
        System.out.println("Building Lexicon and InvertedList...");
        makeLexicon("./result/OneWordPosts");
        makeLexicon("./result/TwoWordPosts");
        makeLexicon("./result/ThreeWordPosts");
